...
- **2026-09-19** — Production auth/deploy repair. Login never persisted a session across the separate frontend/API `workers.dev` origins (`SameSite=Lax` cookie is rejected on cross-site fetch responses) → `/account` bounced to `/login`; cookie is now `SameSite=None; Secure` when deployed, with an `Origin` allow-list as the CSRF check. Frontend `build` script deleted `.open-next/` mid-`opennextjs-cloudflare build` (deploy blocker) — fixed. Verify-email page now distinguishes expired / invalid-or-used links and offers resend; resend endpoint rate-limited and surfaced after a 403 at sign-in. `wrangler` scripts now target explicit environments (bare `wrangler deploy` used localhost `FRONTEND_URL` and a placeholder D1 id); staging `FRONTEND_URL` added; orphan migration and stray `backend/.env.production` removed. Test suite 158 → 175.
