...
- [x] Home, About, Divisions (static content pages; no backend/schema yet — see `docs/modules/events.md` for the pattern a future dynamic Divisions domain would follow)
- [x] News (separate from Events)
- [x] Events — content management + registration/waitlisting done (`docs/modules/events.md` §1–§10). Countdown display, calendar/ICS export, and attendance tracking remain open — see that doc's §11 for why each was deferred rather than guessed at
- [x] Projects (public-facing listing/detail views)
- [x] Awards & Recognition (public view) — content-management model only; badges/certificates/formal Hall of Fame/nomination workflows are Phase 8, see `docs/modules/awards.md` §8
