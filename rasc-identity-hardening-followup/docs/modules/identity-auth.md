...
## 7. Tests
- Unit: `password.test.ts`, `tokens.test.ts`, `require-role.test.ts`,
  `login-policy.test.ts`, `rate-limit-policy.test.ts` — 20 tests, all
  passing, covering hashing correctness/randomness, token
  format/determinism, role-boundary logic, lockout-threshold behavior, and
  fixed-window rate-limit evaluation, all in isolation.
